...
shapiro.test(userdata$score)
plot(density(userdata$score))
#t.test(userdata$score~userdata$group)
res.aov <- aov(userdata$score~userdata$group) # p-value 0.036, Significant even with a small sample. So we can expect even larger effect size with a larger sample size.
summary(res.aov)
TukeyHSD(res.aov)
...
#t.test(userdata$rawscore~userdata$group)
res.aov <- aov(userdata$rawscore~userdata$group) # p-value 0.043, Significant even with a small sample. So we can expect even larger effect size with a larger sample size.
summary(res.aov)
TukeyHSD(res.aov)
...
#t.test(userdata$conf~userdata$group)
res.aov <- aov(userdata$conf~userdata$group) # p-value = 0.79. Not significant. No difference between confidence. Perhaps with more training the confidence values may show difference. 
summary(res.aov)
TukeyHSD(res.aov)
